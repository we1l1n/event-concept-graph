...
df['end'] = df['concept'].apply(lambda x:'/c/en/'+str(x).replace(',','').replace(' ','_'))
df['weight'] = df['probability'].apply(lambda x:x/10000)
df.to_csv('neo-en-noURL/microsoft-rel-*.csv',encoding='utf-8',sep=',',header=False,columns=['start','rel','end','weight'],index=False)
node = df['start'].append(df['end']).drop_duplicates()
node.to_csv('neo-en-noURL/microsoft-node-*.csv',encoding='utf-8',sep=',',header=False,index=False)
from uri import *
import re
weight_re = re.compile('\"weight\":\s(.+)}')
#32755361
...
df = df[df['filter']]
df['weight'] = df['info'].apply(lambda x:float(weight_re.search(x).groups()[0]) if x!= 'foo' else 0.0)
df.to_csv('neo-en-noURL/conceptnet-rel-*.csv',encoding='utf-8',sep=',',header=False,columns=['start','rel','end','weight'],index=False)
node = df['start'].append(df['end']).drop_duplicates()
node.to_csv('neo-en-noURL/conceptnet-node-*.csv',encoding='utf-8',sep=',',header=False,index=False)