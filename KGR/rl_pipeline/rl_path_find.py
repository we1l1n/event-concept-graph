...
def sl_test(episodes=300):
    ...
    print('Success persentage:', success/episodes);f_logs.write('sl_test-Success persentage:'+str(success/episodes)+'\n')
####################################################################################################################
# RL
...
def REINFORCE(train_pairs, policy_nn, num_episodes):
    ...
    for i_episode in range(num_episodes):
        ...
        if done == 1:
            ...
            try:
                good_episodes = teacher(sample[0], sample[1], teacher_num_paths, env, kb)
    ...
    print('Success percentage:',success/num_episodes);f_logs.write('rl_train-Success persentage:'+str(success/num_episodes)+'\n')
    # store path stats
...
def rl_test(episodes=500):
    ...
    with tf.Session() as sess:
        ...
        for episode in range(len(test_pairs) if len(test_pairs)<episodes else episodes):
            ...
            print('Success:',success)
            print('Success persentage:', success/episodes)
    print('Success persentage:', success/episodes);f_logs.write('rl_test-Success persentage:'+str(success/episodes)+'\n')
    # env path reset
    env.path,env.path_relations = [],[]
    # store path to use 
    path_found_relation = [' -> '.join([rel for ix,rel in enumerate(path.split(' -> ')) if ix%2 == 0]) \
...
train_pairs = [line for line in codecs.open(data_path+rel_path+'_train_pos','r',encoding='utf-8')]
